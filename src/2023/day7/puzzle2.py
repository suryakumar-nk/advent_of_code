...
def checkJ(charMap):
    if 'J' in charMap:
        if ((charMap['J'] == 1) or (charMap['J'] == 2)):
            maxKey = ''
            maxVal = 1
            for i in charMap.keys():
                if (i != 'J' and charMap[i] > maxVal):
                    # if (maxKey != '' ): 
                    #     if (letters.index(i) > letters.index(maxKey)):
                    #         maxKey = i
                    # else:
                    maxKey = i
            if (maxKey != ''):
                charMap[maxKey] = charMap[maxKey] + charMap['J']
                charMap.pop('J')
    return charMap
